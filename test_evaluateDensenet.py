...
test_model = "densenet" # model name
...
test_it = test_datagen.flow_from_directory(data_directory, class_mode='categorical', target_size = (224, 224), batch_size = 32)
...
print("Number layers of " + test_model, len(densenet_model.layers))
...
transfer_model = Model(inputs = densenet_model.input, outputs = x)
...
modelPath = './modelStorage/densenet201/'
modelFolder = 'Dec-21-2021-09-50-39' + '/'
modelName = "weights-improvement-11-0.41.hdf5"
    
transfer_model.compile(loss="categorical_crossentropy", optimizer=optimizers.Adam(lr=0.00001), metrics=["accuracy", "Precision", "Recall", "TruePositives", "TrueNegatives", "FalsePositives", "FalseNegatives"])
