...
xception_model = Xception(weights = 'imagenet', include_top = False, input_shape = (200, 200, 3))
...
transfer_model = Model(inputs = xception_model.input, outputs = x)
...
folder_path = "./modelStorage/xception/" + folder_name + "/"
...
transfer_model.compile(loss="categorical_crossentropy", optimizer=optimizers.Adam(lr=0.000001), metrics=["accuracy"])
...
modelName = 'weights-improvement-18-0.43.hdf5'
start_epoch = int(modelName.split('-')[2])
total_epoch = start_epoch + 20
transfer_model.load_weights('./modelStorage/xception/Dec-20-2021-23-53-16/' + modelName)
history = transfer_model.fit(train_it, 
                            epochs = total_epoch, 
                            steps_per_epoch = train_it.samples/batchSize,
                            validation_data = test_it,
